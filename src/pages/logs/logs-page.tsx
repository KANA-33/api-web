...
function LogDetailsView({
  ...
  const channelName = getString(record, ["channel_name"], "9Router");
  ...
  const requestId = getString(
    record,
    ["request_id", "upstream_request_id", "id", "task_id", "mj_id"],
    "unknown-request",
  );
  const token = getString(record, ["token_name", "token", "task_id", "mj_id"], "default");
  const retryCount = getNumber(record, ["retry_count", "retry_times", "retry"]);
  const status = getLogStatus(activeTab, record);
  const path = getRequestPath(activeTab, record);
  const hitTier = getString(record, ["hit_tier", "tier"], "base");
  const inputRate = getString(record, ["input_rate", "prompt_rate"], "¥2.044/M");
  const outputRate = getString(record, ["output_rate", "completion_rate"], "¥12.045/M");
  const multiplier = getString(record, ["multiplier", "group_multiplier"], "1.0000x");
  const source = getString(record, ["billing_source", "source"], "Local Billing");
  const endReason = getString(record, ["end_reason", "finish_reason"], "client_gone");
  const endError = getString(record, ["end_error", "error", "message"], "context canceled");
  const flowStatus = getString(record, ["flow_status", "status"], "error");
      ...
      <div className="mt-10 grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-8">
          ...
          <DetailSection>
            <div className="border border-[#d8d2d2] bg-[#fffdfd]">
              <div className="flex items-start gap-4 border-b border-[#d8d2d2] bg-[#fbf9f9] p-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-[#242121]">Dynamic Pricing</h3>
                  ...
                  <div className="mt-4 overflow-hidden border border-[#d8d2d2]">
                    ...
                    <div className="grid grid-cols-3 bg-[#effcf9] px-4 py-4 text-sm text-[#242121]">
                      <span>
                        ...
                        <small className="mt-1 block text-[#6c6a67]">Length &lt;= 256K</small>
                      </span>
                      <span className="text-right">{inputRate}</span>
                      <span className="text-right">{outputRate}</span>
                    </div>
                    <div className="grid grid-cols-3 border-t border-[#d8d2d2] px-4 py-4 text-sm text-[#242121]">
                      <span>tier_2</span>
                      <span className="text-right">¥8.0300</span>
                      <span className="text-right">¥48.0340</span>
                    </div>
                  </div>
                </div>
          ...
          <DetailSection title="Flow Status">
            <DetailCard className="border-[#efcaca] bg-[#fffafa]">
              <DetailRow
                label="Status"
                value={
        ...
        <aside className="space-y-3 xl:sticky xl:top-8 xl:self-start">
          ...
          <DetailCard className="p-6">
            <DetailRow label="Mode" value={getString(record, ["billing_mode", "mode"], "Dynamic")} />
